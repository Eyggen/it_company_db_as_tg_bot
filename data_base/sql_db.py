...
async def return_data_from_db_by_message(message):
    user = 'U'+str(message.chat.id)
    try:
        data = cur.execute("SELECT * FROM " + user + " WHERE data == '" + str(message.text) + "'").fetchall()
        if len(data) == 0:
            await bot.send_message(message.chat.id, 'У вас немає записів за цю дату')
        else:
            for row in data:
                await bot.send_message(message.chat.id, f'Object: {row[0]}\nPrice: {row[1]}\nDecription: {row[2]}\nTime: {row[3]}, Data: {row[4]}')
    except:
        await bot.send_message(message.chat.id, 'Шось пішло не так, перевірте правильність написання дати.')
...
async def zap1(message, first, second):
    try:
        ...
        if len(data) == 0:
            await bot.send_message(message.chat.id, 'Немає таких записів')
        else:
            for row in data:
                await bot.send_message(message.chat.id, f"Назва компанії: {row[0]}\nРік заснування: {row[1]}\nКількість працівників: {row[2]}\nКількість офісів: {row[3]}")
    except Exception as ex:
...
async def zap2(message, first, second):
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                await bot.send_message(message.chat.id, f"№_Офісу: {row[0]}\nАдреса офісу: {row[1]}\nКількість персоналу: {row[2]}")
    except Exception as ex:
        await bot.send_message(message.chat.id, f"{ex}")
...
async def zap3(message, first, second):
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                await bot.send_message(message.chat.id, f"ПІБ працівника: {row[0]}\nДата народження: {row[1]}\nПосада: {row[2]}")
    except Exception as ex:
        await bot.send_message(message.chat.id, f"{ex}")
...
async def zap4(message, first, second):
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                await bot.send_message(message.chat.id, f"Назва проекту: {row[0]}\nКількість працівників: {row[1]}\nБюджет: {row[2]}")
    except Exception as ex:
        await bot.send_message(message.chat.id, f"{ex}")
...
async def zap5(message, first, second):
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                await bot.send_message(message.chat.id, f"Назва вакансії: {row[0]}\nОпис: {row[1]}\nЗапропонована плата: {row[2]}\nТип занятості: {row[3]}")
    except Exception as ex:
        await bot.send_message(message.chat.id, f"{ex}")
...
async def zap6(message, first, second):
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                await bot.send_message(message.chat.id, f"Номер команди: {row[0]}\nКількість учасників: {row[1]}\nВиділений бюджет: {row[2]}")
    except Exception as ex:
        await bot.send_message(message.chat.id, f"{ex}")
...
async def zap7(message, first, second):
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                await bot.send_message(message.chat.id, f"ПІБ директора: {row[0]}\nЗаробітня плата: {row[1]}\nСтаж роботи: {row[2]}")
    except Exception as ex:
        await bot.send_message(message.chat.id, f"{ex}")
...
async def zap8(message, first, second):
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                await bot.send_message(message.chat.id, f"ПІБ замовника: {row[0]}\nEmail: {row[1]}\nДата першого замовлення: {row[2]}")
    except Exception as ex:
        await bot.send_message(message.chat.id, f"{ex}")

async def zap9(message, first, second):
    try:
        data = cur.execute(f"SELECT [Назва вакансії],[Опис],[Запропонована плата],[Тип занятості] FROM R5 LEFT JOIN R6 ON R6.[Вакансія] = R5.[Назва вакансії] LEFT JOIN R3 ON R3.[ПІБ працівника] = R5.[Працівник] WHERE R6.[Номер команди] = ? AND R3.[Проект] = ?", (int(first), second)).fetchall()
        print(data)
        print([first, second], "zap9")
        if len(data) == 0:
            await bot.send_message(message.chat.id, 'Немає таких записів')
        else:
            for row in data:
...
async def insert(message, table, criter):
    try:
        if table == "R1":
            ...
            #await bot.send_message(message.chat.id, f"Table: {table}\n1: {criter[0]}\n2: {criter[1]}\n3: {criter[2]}\n4: {criter[3]}\n5: {criter[4]}\n6: {criter[5]}") 
            base.commit()
        elif table == "R2":
...
async def select(message, table):
    ...
    try:
        ...
        if lenght_of_data == 0:
            await bot.send_message(message.chat.id, 'Немає записів')
        else:
            i = 0
            lenght_of_row = len(data[0])
            if lenght_of_row == 4:
                for row in data:
                    await bot.send_message(message.chat.id, f'{table_rows[table][0]}: {row[0]}\n{table_rows[table][1]}: {row[1]}\n{table_rows[table][2]}: {row[2]}\n{table_rows[table][3]}: {row[3]}')
